...
export default function SeaMap() {
  return (
    <main className="relative h-screen w-full bg-[#0d2b5e] overflow-hidden flex flex-col items-center p-8 justify-between">
      ...
      {/* 2. HEADER SECTION (Donna is now larger) */}
      <div className="flex flex-col items-center text-center z-10">
        <div className="relative mb-2 animate-bob">
          <Image 
            src="/images/donna.svg" 
            alt="Donna" 
            width={150}  /* Increased from 75 to 110 */
            height={150} 
      ...
      {/* 3. THE ZONE GRID SECTION (Cards are now more compact) */}
      <div className="w-full max-w-4xl z-10 flex flex-col justify-center flex-1">
        ...
        {/* max-w-3xl on the grid container reduces the overall footprint of the 4 boxes */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 px-4 max-w-3xl mx-auto w-full">
          {ZONES.map((zone) => (
            <Link href={zone.href} key={zone.id}>
            <div 
              key={zone.id}
              /* Reduced padding from p-10 to p-6 to make boxes smaller */
